        ...
        if hasattr(markdown, 'version'):
            ...
            if markdown_vers < (1,7):
                ...
                if markdown_vers >= (2,1):
                    if safe_mode:
                        return mark_safe(markdown.markdown(force_text(value), extensions=extensions, safe_mode=safe_mode, enable_attributes=False))
                    else:
                        return mark_safe(markdown.markdown(force_text(value), extensions=extensions, safe_mode=safe_mode))
                else:
                    warnings.warn(python_markdown_deprecation, DeprecationWarning)
                    return mark_safe(markdown.markdown(force_text(value), extensions=extensions, safe_mode=safe_mode))
        else:
